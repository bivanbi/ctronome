...
int main(int argc, char *argv[]) {
    ...
    if (args.is_program) program_file = open_program_file(args.program_file_path);
    ...
    while (args.repeat_count > 0) {
        if (args.is_program) {
            read_next_program_line(&args, &program_file);
        }
    ...
    if (args.is_program) close_program_file(&program_file);
 ...
 */
void read_next_program_line(struct Arguments *args, struct ProgramFile *program_file) {
    struct ProgramLine program_line;
...
struct DspDevice open_sound_device(char *dsp_device_path, struct WavData *wav) {
    log_message(LEVEL_DEBUG, "calling dsp_init(%s)\n", dsp_device_path);

    struct DspDevice dsp_device = dsp_init(dsp_device_path, wav->bits_per_sample, wav->number_of_channels, wav->sample_rate);

    log_message(LEVEL_DEBUG, "dsp channels: '%d', sample rate: '%d', bits per sample: '%d'\n",
