...
void parse_dsp_bits_per_sample(struct DspDevice *dsp_device, word bits_per_sample) {
    ...
    if (bits_per_sample == 8) {
        ...
        log_message(LEVEL_FATAL, "only 8 and 16 bits per sample are supported. (got: %d)\n", bits_per_sample);
        exit(1);
    }
...
void parse_dsp_number_of_channels(struct DspDevice *dsp_device, word number_of_channels) {
    if ((number_of_channels < 1) || (number_of_channels > 2)) {
        log_message(LEVEL_FATAL, "number of channels must be 1 (mono) or 2 (stereo) (got: %d\n", number_of_channels);
        exit(1);
    }
...
void parse_dsp_sample_rate(struct DspDevice *dsp_device, dword sample_rate) {
    if ((sample_rate < 8000) || (sample_rate > 96000)) {
        log_message(LEVEL_FATAL, "sample rate must be between 8000 and 96000 (got: %d\n", sample_rate);
        exit(1);
    }
...
void open_dsp_device(struct DspDevice *dsp_device, char *dsp_device_path) {
    ...
    if ((dsp_device->handler = open(dsp_device_path, O_WRONLY)) == -1) {
        log_message(LEVEL_FATAL, "FATAL: cannot open dsp device %s\n", dsp_device_path);
        log_message(LEVEL_FATAL, "cannot open dsp device %s\n", dsp_device_path);
...
void set_dsp_format(struct DspDevice *dsp_device, char *dsp_device_path) {
    if (ioctl(dsp_device->handler, SNDCTL_DSP_GETFMTS, &supported_dsp_formats) == -1) {
        log_message(LEVEL_FATAL, "unable to get supported formats from '%s'\n", dsp_device_path);
    ...
    if ((ioctl(dsp_device->handler, SNDCTL_DSP_SETFMT, &dsp_device->dsp_format)) == -1) {
        log_message(LEVEL_FATAL, "unable to set output format for %s\n", dsp_device_path);
...
void set_dsp_sample_rate(struct DspDevice *dsp_device, char *dsp_device_path) {
    ...
    if (ioctl(dsp_device->handler, SNDCTL_DSP_SPEED, &dsp_device->sample_rate) == -1) {
        log_message(LEVEL_FATAL, "unable to set sample rate for %s\n", dsp_device_path);
...
void set_dsp_number_of_channels(struct DspDevice *dsp_device, char *dsp_device_path) {
    ...
    if (ioctl(dsp_device->handler, SNDCTL_DSP_CHANNELS, &dsp_device->number_of_channels) == -1) {
        log_message(LEVEL_FATAL, "unable to set no. of channels for %s\n", dsp_device_path);
...
void verify_dsp_device(struct DspDevice *dsp_device, word bits_per_sample, word number_of_channels, dword sample_rate) {
    if ((bits_per_sample == 8) && (dsp_device->dsp_format != dsp_8bit_unsigned_format)) {
        log_message(LEVEL_FATAL, "your dsp device does not seem to support unsigned 8 bit (AFMT_8U) format\n");
        exit(1);
    }

    if ((bits_per_sample == 16) && (dsp_device->dsp_format != dsp_16bit_signed_format)) {
        log_message(LEVEL_FATAL, "your dsp device does not seem to support signed 16 bit (AFMT_S16_LE) format\n");
        exit(1);
    }

    if (dsp_device->number_of_channels != number_of_channels) {
        log_message(LEVEL_FATAL, "your wav has %d channels, while your DSP only supports %d channels.\n", number_of_channels, dsp_device->number_of_channels);
        exit(1);
    }

    if (dsp_device->sample_rate != sample_rate) {
        log_message(LEVEL_DEBUG, "dsp: anticipated sample rate and actual dsp sample rate differs, will sound funny\n");
    }
}
...
void dsp_write(byte dsp_handle, byte *from, dword count) {
    ...
    if (bytes_written < count) {
        log_message(LEVEL_FATAL, "tried to write %d bytes to dsp device,\nonly %d is written\n", count, bytes_written);
        exit(1);
    }
}
